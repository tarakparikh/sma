...
class symbol_analysis:
	...
	def _analyze(self):
		...
		name = self._name
		name += " %6s " % (self._curPrice)
		if (self._curPrice > self._prevPrice):
		    name += "+"
		    name += "%.4s" %(self._curPrice-self._prevPrice)
		else:
		    name += "-"
		    name += "%.4s" %(self._prevPrice-self._curPrice)
		...
		retString = "%-6s %45s %-20s %-23s %s %2s" % (self.symbol, name, trendStr, smaStr, trendShape, self._200dayPct)
		return retString
...
#
	...
	def run_program(self):
		...
		self.run_portfolio_analysis(self.portfolio_file)
	    else:
