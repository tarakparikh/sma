...
class ftpTransfer(object):
	...
	def handleTransfer(self,block):
    		self.file.write(block)
    		print ".",
    

	def findLatest(self,line):
    		match = re.match('.*athdl_linux_RHEL3_5.0_A1_(\d+)_19_2012.*', line)
    		if match:
			self.relList.append(line)


	def getLatest(self):
		ftp1 = FTP(self.connection)
		ftp1.login("athdl","a1t9h9d9l")
		ftp1.cwd("RELEASES")
		ftp1.dir(self.findLatest)
		for rel in self.relList:
			match = re.match('.*athdl_linux_RHEL3_5.0_A1_(\d+)_19_2012.*', rel)
			if match:
				relNum = match.group(1)
				if (relNum > self.lastRel):
					self.lastRel = relNum 

		filename = 'athdl_linux_RHEL3_5.0_A1_%s_19_2012.tgz' % (self.lastRel)
		print "Get ", filename
		self.file = open (filename,'wb')
		ftp1.retrbinary('RETR ' + filename, self.handleTransfer)
		ftp1.close()
